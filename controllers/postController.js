...
const createPost = async (req, res) => {
    try {
        ...
        if (!postedBy || !text) {
            return res.status(400).json({ error: "posted by and text field are required" })
        }
        ...
        let newPost = new Post({ postedBy, text, img })

        await newPost.save()

        newPost = await newPost.populate("postedBy", "username");
